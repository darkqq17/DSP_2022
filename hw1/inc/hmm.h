...
typedef struct{
    char *model_name;
    int state_num;					//number of state; Q={1,2,...,N}
    int observ_num;					//number of observation; V={1,2,...,M}
    double initial[MAX_STATE];			//initial prob.
    double transition[MAX_STATE][MAX_STATE];	//transition prob.; a[i][j] is the transition probof going from state i at time t to state j at time t+1
    double observation[MAX_OBSERV][MAX_STATE];	//observation prob.; b[j][k] is the probability of observing symbol k in state j
} HMM;
...
static void loadHMM( HMM *hmm, const char *filename )
{
    ...
    while( fscanf( fp, "%s", token ) > 0 )
    {
        ...
        if( strcmp( token, "initial:" ) == 0 ){
            n = fscanf(fp, "%d", &hmm->state_num );

            for( i = 0 ; i < hmm->state_num ; i++ )
                n = fscanf(fp, "%lf", &( hmm->initial[i] ) );
        }
            ...
            for( i = 0 ; i < hmm->state_num ; i++ )
                for( j = 0 ; j < hmm->state_num ; j++ )
                    n = fscanf(fp, "%lf", &( hmm->transition[i][j] ));
        }
        else if( strcmp( token, "observation:" ) == 0 ){
            n = fscanf(fp, "%d", &hmm->observ_num );

            for( i = 0 ; i < hmm->observ_num ; i++ )
                for( j = 0 ; j < hmm->state_num ; j++ )
                    n = fscanf(fp, "%lf", &( hmm->observation[i][j]) );
        }
    }
}
